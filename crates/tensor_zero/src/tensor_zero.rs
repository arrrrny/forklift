...
fn format_curl_command(api_url: &str, request: &Request) -> String {
    let uri = format!("{api_url}/chat/completions");
    let body = serde_json::to_string(request).unwrap_or_else(|_| "{}".to_string());
    
    format!(
        "curl -X POST \"{}\" \\\n  -H \"Content-Type: application/json\" \\\n  -H \"HTTP-Referer: https://zed.dev\" \\\n  -H \"X-Title: Zed Editor\" \\\n  -d '{}'",
        uri, body
    )
}

pub const TENSOR_ZERO_API_URL: &str = "http://localhost:3000/openai/v1";
...
impl Model {
    pub fn default_fast() -> Self {
        Self::new(
            "tensorzero::model_name::openai::gpt_4o_mini",
            Some("GPT-4o Mini"),
            Some(128000),
            Some(true),
            Some(false),
    ...
    pub fn new(
        ...
        Self {
            ...
            max_tokens: max_tokens.unwrap_or(128000),
            supports_tools,
...
pub async fn complete(
    ...
    if response.status().is_success() {
        ...
        #[derive(Deserialize)]
        struct TensorZeroError {
            message: String,
            #[serde(default)]
            code: String,
        }

        match serde_json::from_str::<TensorZeroResponse>(&body) {
            Ok(response) if !response.error.message.is_empty() => {
                ...
                Err(anyhow!(
                    "Failed to connect to TensorZero API: {}\n\nCurl command to reproduce:\n{}",
                    error_message,
                    format_curl_command(api_url, &request_body)
                ))
            }
            _ => Err(anyhow!(
                "Failed to connect to TensorZero API: {} {}\n\nCurl command to reproduce:\n{}",
        ...
        match serde_json::from_str::<TensorZeroResponse>(&body) {
            Ok(response) if !response.error.message.is_empty() => {
                ...
                Err(anyhow!(
                    "Failed to connect to TensorZero API: {}\n\nCurl command to reproduce:\n{}",
                    error_message,
                    format_curl_command(api_url, &request)
                ))
            }
            _ => Err(anyhow!(
                "Failed to connect to TensorZero API: {} {}\n\nCurl command to reproduce:\n{}",
...
pub fn count_tensor_zero_tokens(content: &str) -> usize {
    ...
    // This is a rough approximation, similar to the OpenRouter implementation
    (content.len() + 3) / 4
}