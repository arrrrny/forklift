...
pub const LITELLM_API_URL: &str = "http://localhost:4000/v1";
...
pub async fn complete(
    ...
    if response.status().is_success() {
        ...
        match serde_json::from_str::<LiteLLMResponse>(&body) {
...
pub async fn list_models(client: &dyn HttpClient, api_url: &str) -> Result<Vec<Model>> {
    ...
    if response.status().is_success() {
        let response: ListModelsResponse =
            serde_json::from_str(&body).context("Unable to parse LiteLLM models response")?;
