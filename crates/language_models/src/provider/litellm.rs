...
impl LiteLLMLanguageModel {
    fn stream_completion_internal(
        ...
        cx: &AsyncApp,
    ) -> BoxFuture<'static, Result<BoxStream<'static, Result<litellm::StreamResponse>>>> {
        let http_client = self.http_client.clone();
...
fn map_litellm_to_events(
    stream: BoxStream<'static, Result<litellm::StreamResponse>>,
                    ...
                    Ok(response) => {
                        ...
                        for choice in response.choices {
                            ...
                            match finish_reason.as_deref() {
                                ...
                                Some(reason) => {
                                    log::error!("Unexpected LiteLLM finish reason: {reason}");
                                    events.push(Ok(LanguageModelCompletionEvent::Stop(
                                        StopReason::EndTurn,
                                    )));
                                }
...
impl LanguageModel for LiteLLMLanguageModel {
    ...
    fn stream_completion(
        ...
        >,
    > {
        let litellm_request = into_litellm(
            request,
            self.model.id().to_string(),
            self.max_output_tokens(),
        );
        let stream = self.stream_completion_internal(litellm_request, cx);

        async move {
            let stream = stream.await?;
            Ok(map_litellm_to_events(stream).boxed())
        }
        .boxed()
    }
            ...
            match content {
                ...
                language_model::MessageContent::ToolUse(tool_use) => {
                    ...
                    if let Some(last_assistant_message) = messages.iter_mut().rfind(|message| {
                        matches!(message, litellm::RequestMessage::Assistant { .. })
                    }) {
                        if let litellm::RequestMessage::Assistant { tool_calls, .. } =
...
impl ConfigurationView {
    fn new(state: Entity<State>, window: &mut Window, cx: &mut Context<Self>) -> Self {
        let api_key_editor = cx.new(|cx| {
            ...
            // Use a longer, more realistic litellm API key placeholder
