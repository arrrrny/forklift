...
async fn test_editorconfig_support(cx: &mut gpui::TestAppContext) {
    ...
    let worktree = project.update(cx, |project, model, cx| project.worktrees(cx).next().unwrap());
...
async fn test_managing_language_servers(cx: &mut gpui::TestAppContext) {
    ...
    rust_buffer.update(cx, |buffer, model, cx| buffer.edit([(16..16, "2")], None, cx));
    assert_eq!(
        fake_rust_server
    ...
    toml_buffer.update(cx, |buffer, model, cx| buffer.edit([(5..5, "23")], None, cx));
    ...
    project
        .update(cx, |project, model, cx| project.save_buffer(toml_buffer, cx))
        .await
        .unwrap();
    assert_eq!(
        fake_rust_server
    ...
    rust_buffer2.update(cx, |buffer, model, cx| buffer.edit([(0..0, "// ")], None, cx));
    assert_eq!(
        fake_json_server
            .receive_notification::<lsp::notification::DidChangeTextDocument>()
            .await
...
async fn test_single_file_worktrees_diagnostics(cx: &mut gpui::TestAppContext) {
    ...
    let buffer_b = project
        .update(cx, |project, model, cx| project.open_local_buffer("/dir/b.rs", cx))
        .await
        .unwrap();
...
async fn test_disk_based_diagnostics_progress(cx: &mut gpui::TestAppContext) {
    ...
    let worktree_id = project.update(cx, |p, model, cx| p.worktrees(cx).next().unwrap().read(cx).id());
...
async fn test_toggling_enable_language_server(cx: &mut gpui::TestAppContext) {
    ...
    let _js_buffer = project
        .update(cx, |project, model, cx| project.open_local_buffer("/dir/b.js", cx))
        .await
        .unwrap();
...
async fn test_transforming_diagnostics(cx: &mut gpui::TestAppContext) {
    ...
    buffer.update(cx, |buffer, model, cx| buffer.edit([(0..0, "\n\n")], None, cx));
...
async fn test_save_file(cx: &mut gpui::TestAppContext) {
    ...
    assert_eq!(new_text, buffer.update(cx, |buffer, model, _| buffer.text()));
}
...
async fn test_save_as(cx: &mut gpui::TestAppContext) {
    ...
    let buffer = project.update(cx, |project, model, cx| project.create_local_buffer("", None, cx));
    buffer.update(cx, |buffer, model, cx| {
...
async fn test_rescan_and_remote_updates(cx: &mut gpui::TestAppContext) {
    ...
    let buffer_for_path = |path: &'static str, cx: &mut gpui::TestAppContext| {
        let buffer = project.update(cx, |p, model, cx| p.open_local_buffer(dir.path().join(path), cx));
    ...
    let tree = project.update(cx, |project, model, cx| project.worktrees(cx).next().unwrap());
