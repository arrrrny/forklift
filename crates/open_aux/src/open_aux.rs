...
pub const OPEN_AUX_API_URL: &str = "http://localhost:4001/v1";
...
pub async fn complete(
    ...
    if response.status().is_success() {
        ...
        match serde_json::from_str::<OpenAuxResponse>(&body) {
            Ok(response) if !response.error.message.is_empty() => {
                ...
                Err(anyhow!(
                    "Failed to connect to OpenAux API: {}",
                    error_message
                ))
            }
            _ => Err(anyhow!(
...
pub async fn list_models(client: &dyn HttpClient, api_url: &str) -> Result<Vec<Model>> {
    ...
    if response.status().is_success() {
        let response: ListModelsResponse =
            serde_json::from_str(&body).context("Unable to parse OpenAux models response")?;
