...
fn test_clone(cx: &mut TestAppContext) {
    ...
    let cloned_editor = editor
        .update(cx, |editor, model, cx| {
            cx.open_window(Default::default(), |cx| cx.new_model(|model, cx| editor.clone(cx)))
        })
        .unwrap()
        .unwrap();

    let snapshot = editor.update(cx, |e, model, cx| e.snapshot(cx)).unwrap();
    let cloned_snapshot = cloned_editor.update(cx, |e, model, cx| e.snapshot(cx)).unwrap();

    assert_eq!(
        cloned_editor
            .update(cx, |e, model, cx| e.display_text(cx))
            .unwrap(),
        editor.update(cx, |e, model, cx| e.display_text(cx)).unwrap()
    );
    assert_eq!(
        cloned_snapshot
    ...
    assert_set_eq!(
        cloned_editor
            .update(cx, |editor, model, cx| editor.selections.ranges::<Point>(cx))
            .unwrap(),
        editor
...
fn test_indent_outdent_with_excerpts(cx: &mut TestAppContext) {
    ...
    let toml_buffer =
        cx.new_model(|model, cx| Buffer::local("a = 1\nb = 2\n", cx).with_language(toml_language, cx));
...
async fn test_select_larger_smaller_syntax_node(cx: &mut gpui::TestAppContext) {
    ...
    let buffer = cx.new_model(|model, cx| Buffer::local(text, cx).with_language(language, cx));
    let buffer = cx.new_model(|model, cx| MultiBuffer::singleton(buffer, cx));
...
async fn test_autoindent_selections(cx: &mut gpui::TestAppContext) {
...

    {
        ...
        let buffer = cx.update_editor(|editor, cx| {
            ...
            buffer.update(cx, |buffer, model, cx| buffer.set_language(Some(rust_lang()), cx));
            buffer
        });
...
async fn test_document_format_during_save(cx: &mut gpui::TestAppContext) {
    ...
    let buffer = project
        .update(cx, |project, model, cx| project.open_local_buffer("/file.rs", cx))
        .await
        .unwrap();
    ...
    let buffer = cx.new_model(|model, cx| MultiBuffer::singleton(buffer, cx));
    let (editor, cx) = cx.add_window_view(|cx| build_editor(buffer, cx));
    editor.update(cx, |editor, model, cx| editor.set_text("one\ntwo\nthree\n", cx));
    assert!(cx.read(|cx| editor.is_dirty(cx)));

    let save = editor
        .update(cx, |editor, model, cx| editor.save(true, project.clone(), cx))
        .unwrap();
    fake_server
    ...
    editor.update(cx, |editor, model, cx| editor.set_text("somehting_new\n", cx));
    assert!(cx.read(|cx| editor.is_dirty(cx)));
    let save = editor
        .update(cx, |editor, model, cx| editor.save(true, project.clone(), cx))
        .unwrap();
...
async fn test_document_format_with_prettier(cx: &mut gpui::TestAppContext) {
    ...
    let buffer = project
        .update(cx, |project, model, cx| project.open_local_buffer("/file.ts", cx))
        .await
        .unwrap();
...
fn test_crease_insertion_and_rendering(cx: &mut TestAppContext) {
    ...
    cx.update_window(*editor, |_, cx| (render_args.callback)(true, cx))
        .unwrap();
    let snapshot = editor.update(cx, |editor, model, cx| editor.snapshot(cx)).unwrap();
    assert!(snapshot.is_line_folded(MultiBufferRow(1)));
