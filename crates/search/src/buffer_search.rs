...
impl Render for BufferSearchBar {
    fn render(&mut self, model: &Model<Self>, cx: &mut AppContext) -> impl IntoElement {
        ...
        if self.query_editor.update(cx, |query_editor, model, cx| {
            query_editor.placeholder_text(cx).is_none()
        }) {
            self.query_editor.update(cx, |editor, model, cx| {
                editor.set_placeholder_text("Search…", cx);
            });
        }

        self.replacement_editor.update(cx, |editor, model, cx| {
...
impl BufferSearchBar {
    ...
    pub fn dismiss(&mut self, _: &Dismiss, model: &Model<Self>, cx: &mut AppContext) {
        self.dismissed = true;
    ...
    pub fn deploy(&mut self, deploy: &Deploy, model: &Model<Self>, cx: &mut AppContext) -> bool {
        if self.show(cx) {
    ...
    pub fn toggle(&mut self, action: &Deploy, model: &Model<Self>, cx: &mut AppContext) {
    ...
    pub fn show(&mut self, model: &Model<Self>, cx: &mut AppContext) -> bool {
    ...
    pub fn search_suggested(&mut self, model: &Model<Self>, cx: &mut AppContext) {
        let search = self
            .query_suggestion(cx)
    ...
    pub fn activate_current_match(&mut self, model: &Model<Self>, cx: &mut AppContext) {
        if let Some(match_ix) = self.active_match_index {
    ...
    pub fn select_query(&mut self, model: &Model<Self>, cx: &mut AppContext) {
    ...
    pub fn query_suggestion(&mut self, model: &Model<Self>, cx: &mut AppContext) -> Option<String> {
        self.active_searchable_item
            .as_ref()
    ...
    pub fn set_replacement(
        ...
        self.replacement_editor
            .update(cx, |replacement_editor, model, cx| {
                replacement_editor
                    .buffer()
                    .update(cx, |replacement_buffer, model, cx| {
    ...
    pub fn search(
        ...
        if updated {
            self.query_editor.update(cx, |query_editor, model, cx| {
                query_editor.buffer().update(cx, |query_buffer, model, cx| {
    ...
    pub fn focus_editor(&mut self, _: &FocusEditor, model: &Model<Self>, cx: &mut AppContext) {
    ...
    pub fn select_last_match(&mut self, model: &Model<Self>, cx: &mut AppContext) {
        if let Some(searchable_item) = self.active_searchable_item.as_ref() {
            if let Some(matches) = self
                .searchable_items_with_matches
                .get(&searchable_item.downgrade())
    ...
    fn on_query_editor_event(
        ...
        match event {
            ...
            editor::EditorEvent::Edited { .. } => {
                ...
                let width = editor.update(cx, |editor, model, cx| {
    ...
    fn toggle_whole_word(&mut self, _: &ToggleWholeWord, model: &Model<Self>, cx: &mut AppContext) {
        self.toggle_search_option(SearchOptions::WHOLE_WORD, cx)
    }

    fn toggle_selection(&mut self, _: &ToggleSelection, model: &Model<Self>, cx: &mut AppContext) {
        if let Some(active_item) = self.active_searchable_item.as_mut() {
    ...
    fn toggle_regex(&mut self, _: &ToggleRegex, model: &Model<Self>, cx: &mut AppContext) {
    ...
    pub fn update_match_index(&mut self, model: &Model<Self>, cx: &mut AppContext) {
        let new_index = self
            .active_searchable_item
            .as_ref()
            .and_then(|searchable_item| {
    ...
    fn tab(&mut self, _: &Tab, model: &Model<Self>, cx: &mut AppContext) {
    ...
    fn tab_prev(&mut self, _: &TabPrev, model: &Model<Self>, cx: &mut AppContext) {
    ...
    fn focus(&self, handle: &gpui::FocusHandle, model: &Model<Self>, cx: &mut AppContext) {
    ...
    pub fn replace_all(&mut self, _: &ReplaceAll, model: &Model<Self>, cx: &mut AppContext) {
        if !self.dismissed && self.active_search.is_some() {
            if let Some(searchable_item) = self.active_searchable_item.as_ref() {
                if let Some(query) = self.active_search.as_ref() {
                    if let Some(matches) = self
    ...
    pub fn match_exists(&mut self, model: &Model<Self>, cx: &mut AppContext) -> bool {
    ...
    fn smartcase(&mut self, model: &Model<Self>, cx: &mut AppContext) {
...
mod tests {
    ...
    fn init_test(
        ...
        let editor = cx.new_model(|model, cx| Editor::for_buffer(buffer.clone(), None, cx));
    ...
    async fn test_search_simple(cx: &mut TestAppContext) {
        ...
        search_bar
            .update(cx, |search_bar, model, cx| {
                search_bar.search("us", None, cx)
            })
            .await
            .unwrap();
        ...
        search_bar.update(cx, |search_bar, model, cx| {
            ...
            assert_eq!(
                editor.update(cx, |editor, model, cx| editor.selections.display_ranges(cx)),
                [DisplayPoint::new(DisplayRow(0), 41)..DisplayPoint::new(DisplayRow(0), 43)]
            );
        });
        search_bar.update(cx, |search_bar, model, _| {
            assert_eq!(search_bar.active_match_index, Some(0));
        });
    ...
    async fn test_search_select_all_matches(cx: &mut TestAppContext) {
        ...
        let buffer = cx.new_model(|model, cx| Buffer::local(buffer_text, cx));
        ...
        window
            .update(cx, |_, model, cx| {
                search_bar.update(cx, |search_bar, model, cx| search_bar.search("a", None, cx))
            })
            .unwrap()
            .await
            .unwrap();
        let initial_selections = window
            .update(cx, |_, model, cx| {
                ...
                let initial_selections = editor.update(cx, |editor, model, cx| {
                ...
                search_bar.update(cx, |this, model, cx| this.select_next_match(&SelectNextMatch, cx));
                initial_selections
            }).unwrap();
        ...
        window
            .update(cx, |_, model, cx| {
                ...
                assert!(
                    editor.update(cx, |this, model, cx| !this.is_focused(cx.window_context())),
    ...
    async fn test_search_query_with_match_whole_word(cx: &mut TestAppContext) {
        ...
        search_bar.update(cx, |_, model, cx| {
            let all_selections =
                editor.update(cx, |editor, model, cx| editor.selections.display_ranges(cx));
            assert_eq!(
                all_selections.len(),
    ...
    async fn test_search_query_history(cx: &mut TestAppContext) {
        ...
        search_bar
            .update(cx, |search_bar, model, cx| search_bar.search("a", None, cx))
            .await
            .unwrap();
        search_bar
            .update(cx, |search_bar, model, cx| search_bar.search("b", None, cx))
            .await
            .unwrap();
        search_bar
            .update(cx, |search_bar, model, cx| {
    ...
    async fn test_replace_simple(cx: &mut TestAppContext) {
        ...
        assert_eq!(
            editor.update(cx, |this, model, cx| { this.text(cx) }),
            r#"
