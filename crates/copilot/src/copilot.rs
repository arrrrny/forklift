...
pub fn init(
    ...
    let copilot = cx.new_model({
        ...
        move |cx| Copilot::start(new_server_id, http, node_runtime, model, model, cx)
    });
    Copilot::set_global(copilot.clone(), model, model, cx);
...
mod tests {
    ...
    async fn test_buffer_management(cx: &mut TestAppContext) {
        ...
        let buffer_1_uri: lsp::Url = format!("buffer://{}", buffer_1.entity_id().as_u64())
            .parse()
            .unwrap();
        copilot.update(cx, |copilot, model, cx| copilot.register_buffer(&buffer_1, model, cx));
        assert_eq!(
        ...
        let buffer_2_uri: lsp::Url = format!("buffer://{}", buffer_2.entity_id().as_u64())
            .parse()
            .unwrap();
        copilot.update(cx, |copilot, model, cx| copilot.register_buffer(&buffer_2, model, cx));
        assert_eq!(
            lsp.receive_notification::<lsp::notification::DidOpenTextDocument>()
                .await,
            lsp::DidOpenTextDocumentParams {
        ...
        buffer_1.update(cx, |buffer, model, cx| buffer.edit([(5..5, " world")], None, model, cx));
        assert_eq!(
