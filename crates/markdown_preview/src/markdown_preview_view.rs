...
impl MarkdownPreviewView {
    pub fn register(workspace: &mut Workspace, model: &Model<>Workspace, _cx: &mut AppContext) {
        workspace.register_action(move |workspace, _: &OpenPreview, cx| {
            if let Some(editor) = Self::resolve_active_item_as_markdown_editor(workspace, model, model, cx) {
                let view = Self::create_markdown_view(workspace, editor, model, model, cx);
        ...
        workspace.register_action(move |workspace, _: &OpenPreviewToTheSide, cx| {
            if let Some(editor) = Self::resolve_active_item_as_markdown_editor(workspace, model, model, cx) {
                let view = Self::create_markdown_view(workspace, editor.clone(), model, model, cx);
                let pane = workspace
                    .find_pane_in_direction(workspace::SplitDirection::Right, model, model, cx)
                    .unwrap_or_else(|| {
                ...
                pane.update(cx, |pane, model, cx| {
                    if let Some(existing_view_idx) = Self::find_existing_preview_item_idx(model, pane) {
                        pane.activate_item(existing_view_idx, true, true, model, model, model, cx);
                    } else {
        ...
        let subscription = cx.subscribe(&editor, |this, editor, event: &EditorEvent, cx| {
            match event {
                ...
                EditorEvent::SelectionsChanged { .. } => {
                    let selection_range =
                        editor.update(cx, |editor, model, cx| editor.selections.last::<usize>(cx).range());
    ...
    fn parse_markdown_in_background(
        ...
        cx.spawn(move |view, mut cx| async move {
            ...
            let (contents, file_location) = view.update(&mut cx, |_, cx| {
                ...
                let file_location = MarkdownPreviewView::get_folder_for_active_editor(editor, model, cx);
